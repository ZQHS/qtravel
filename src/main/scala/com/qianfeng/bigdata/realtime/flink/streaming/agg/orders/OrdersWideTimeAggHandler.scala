...
import com.qianfeng.bigdata.realtime.flink.streaming.funs.orders.OrdersAggFun.{OrderWideTimeAggFun, OrderWideTimeWindowFun}
...
import com.qianfeng.bigdata.realtime.flink.streaming.schema.OrderWideGroupKSchema
...
/**
 * 旅游产品订单宽表数据实时统计计算
 * 1、数据源--travel_orders_ods
 * 2、基于产品类型(productType)+出境类型(toursimType)维度，实时统计orders, maxFee, totalFee, members
 * 3、结果集--kafka
 */
