package com.qianfeng.bigdata.realtime.flink.streaming.funs.orders
...
object OrdersAggFun {
  ...
  class OrderStatisWindowProcessFun extends ProcessWindowFunction[OrderDetailData, OrderDetailStatisData, OrderDetailSessionDimData, TimeWindow]{
    ...
    override def open(parameters: Configuration): Unit = {
      //订单用户列表的状态数据  createTypeInformation[mutable.Set[String]]
      usersStateDesc = new ValueStateDescriptor[mutable.Set[String]](ORDER_STATE_USER_DESC,
        TypeInformation.of(new typeinfo.TypeHint[mutable.Set[String]] {}),
  ...
  class OrderCustomerStatisKeyedProcessFun(maxCount :Long, maxInterval :Long, timeUnit:TimeUnit) extends KeyedProcessFunction[OrderWideAggDimData, OrderWideData, OrderWideCustomerStatisData] {
    ...
    override def open(parameters: Configuration): Unit = {
      ...
      //状态数据：订单UV
      customerUserStateDesc = new ValueStateDescriptor[mutable.Set[String]](CUSTOMER_ORDER_STATE_USER_DESC,
        TypeInformation.of(new typeinfo.TypeHint[mutable.Set[String]] {}),
        new mutable.HashSet[String]())
