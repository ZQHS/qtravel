package com.qianfeng.bigdata.realtime.flink.streaming.sink.orders
...
import com.qianfeng.bigdata.realtime.flink.util.es.ES6ClientUtil
  ...
  def handleData(idxName :String, idxTypeName :String, esID :String,
    ...
    for ((k :String, v:Object) <- value ) { //if(null != k); if(null != v)
      if(QRealTimeConstant.KEY_CT.equals(k)){
        params.put(k, DateUtil.tranTimeToString(v.toString.toLong))
      } else {
        params.put(k, v)
      }
