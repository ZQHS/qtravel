package com.qianfeng.bigdata.realtime.flink.streaming.agg.orders

import java.util.Date
import java.util.concurrent.TimeUnit

import com.qianfeng.bigdata.realtime.flink.constant.QRealTimeConstant
import com.qianfeng.bigdata.realtime.flink.streaming.funs.orders.OrdersAggFun.OrderCustomerStatisKeyedProcessFun
import com.qianfeng.bigdata.realtime.flink.streaming.funs.orders.OrdersETLFun.OrderWideBCFunction
...
import com.qianfeng.bigdata.realtime.flink.util.DateUtil
 ...
 * 旅游产品订单业务
 * 订单数据综合统计
 * 1、数据源--travel_oders_ods
 * 2、自定义状态进行统计，按照订单数量触发创统计，
 * 3、根据维度：产品类型(productType)，产品种类(出境|非出境)(toursimType)，，统计totalOrders, users, totalFee三个值
 * 4、将结果打入es中
 */
object OrdersCustomerStatisHandler {
  ...
  def handleOrdersWideStatisCustomerJob(appName:String, groupID:String, fromTopic:String, indexName:String,maxCount :Long, maxInterval :Long):Unit = {
    try{
       ...
       *   (3) 产品维表广播数据流
       */
      val productDimFieldTypes :List[TypeInformation[_]] = QRealTimeDimTypeInformation.getProductDimFieldTypeInfos()
      //mysql查询sql
       ...
       */
      val statisDStream:DataStream[OrderWideCustomerStatisData] = orderWideDStream
        .keyBy(
        (wide:OrderWideData) => {
      ...
      val esDStream:DataStream[String] = statisDStream.map(
        (value : OrderWideCustomerStatisData) => {
          ...
          //修改时间为yyyy-MM-dd'T'HH:mm:ssZ
          if(result.containsKey("startWindowTime")){
            result += ("startWindowTime"->DateUtil.tranTimeToString(result.get("startWindowTime").toString.toLong))
          }
