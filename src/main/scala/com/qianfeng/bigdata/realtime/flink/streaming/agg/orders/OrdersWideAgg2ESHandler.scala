...
import com.qianfeng.bigdata.realtime.flink.streaming.rdo.QRealTimeDO.{OrderDetailData, OrderWideData}
...
import com.qianfeng.bigdata.realtime.flink.streaming.sink.orders.OrdersWideAggESSink
...
object OrdersWideAgg2ESHandler {
  ...
  def handleOrdersWideAccJob(appName:String, groupID:String,fromTopic:String,  indexName:String):Unit = {
    try{
      ...
      //产品维表广播数据流
      val dimProductBCStream :BroadcastStream[ProductDimDO] = productDS
        .broadcast(productMSDesc)
