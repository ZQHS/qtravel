package com.qf.bigdata.realtime.util.db;
...
import com.qf.bigdata.realtime.util.json.JsonHelper;
...
public class DBDruid implements Serializable {
    ...
    public List<Row> readRecords(String sql, String schema) throws SQLException {
        ...
        if(null != conn){
            ...
            while(rs.next()){
                ...
                String[] fieldNames = exeSchema.split(CommonConstant.COMMA);
    ...
    public static void main(String[] args) throws Exception{
        ...
        String driver = dbProperties.getProperty(TravelConstant.FLINK_JDBC_DRIVER_MYSQL_KEY);
        String url  = dbProperties.getProperty(TravelConstant.FLINK_JDBC_URL_KEY);
        String user = dbProperties.getProperty(TravelConstant.FLINK_JDBC_USERNAME_KEY);
        String passwd = dbProperties.getProperty(TravelConstant.FLINK_JDBC_PASSWD_KEY);

        DBDruid dbDruid = new DBDruid(driver, url, user, passwd);
        String sql = "select product_id, product_level, product_type, departure_code, des_city_code, toursim_tickets_type from qf_nshop.t_product";
...
//        Map<String,Row> results = dbDruid.execSQL(sql, schema, pk);
...
//
//            String productLevel = value.getField(0).toString();
//            String productType = value.getField(2).toString();
//            String toursimType = value.getField(5).toString();
//        }
