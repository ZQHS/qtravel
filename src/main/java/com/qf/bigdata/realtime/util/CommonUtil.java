...
public class CommonUtil {
	...
	//地区
	public static List<Map<String,String>> areaCodes = new ArrayList<Map<String,String>>();
	...
	public static String getRandomIDCard() {
		List<String> citys = Arrays.asList("110101","110102","110105","110106","110107","110108","110109");
		List<String> years = getRangeNumber(1960,2000,1);
		List<String> months = getRangeNumber(1, 12, 1, 2, "0");
		List<String> days = getRangeNumber(1, 31, 1, 2, "0");

		String city = getRandomElementRange(citys);
		String year = getRandomElementRange(years);
		String month = getRandomElementRange(months);
		String day = getRandomElementRange(days);
		String last = getRandomNumStr(4);
		String idcard = city+year+month+day+last;
		return idcard;
	}
	 ...
	 * @return
	 */
	public static int getWinPrice(int lastPrice){
		int percent = getRandomNum(2);
		//int lprice = Integer.valueOf(lastPrice);
		int price = lastPrice * percent / 100;
		return price;
	}
	...
	public static List<String> getRangeNumber(int begin, int end, int sep,String pre){
		List<String> rangeNums = new ArrayList<String>();
		for(int i=begin; i<=end; i+=sep){
			rangeNums.add(pre + String.valueOf(i));
		}
		return rangeNums;
	}
	...
	public static List<String> getRangeNumber(int begin, int end, int sep, int length, String fill){
		List<String> rangeNums = new ArrayList<String>();
		for(int i=begin; i<=end; i+=sep){
			String num = String.valueOf(i);
			int size = num.length();
			if(size < length){
				num = StringUtils.repeat(fill, length-size)+num;
			}
			rangeNums.add(num);
		}
		return rangeNums;
	...
	public static long getRandomTimestamp(String cTimes, String formater){
	...
	public static List<Map<String,String>> getCitys(){
		try{
			if(CollectionUtils.isEmpty(areaCodes)){
				areaCodes = CSVUtil.readCSVFile(CSVUtil.AREA_CODE_GIS_LOCATION_CHINA_FILE, CSVUtil.QUOTE_COMMON);
			}
		}catch (Exception e){
			e.printStackTrace();
		}
